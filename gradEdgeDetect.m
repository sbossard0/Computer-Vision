function gradEdgeDetect(im)
    ...
    %% 1.3 Compute Magnitude & Threshold
    %Compute Mag
    ...
    %If magn(x,y) > thr then possible edge (display as white i.e. 255)
    %If magn(x,y) <= thr then not edge (display as black i.e. 0)
    for ii=1:size(mag_img,1)
        for jj=1:size(mag_img,2)     
            if(mag_img(ii,jj) <= thr)
                thr_img(ii,jj) = 0;
            else
                thr_img(ii,jj) = 255;
            end
    ...
    steps = [0 22.5 45 77.5 90 112.5 135 157.5];
        ...
        for jj=1:size(dir_img,2) 
          ...
          elseif dir_img(ii,jj) > steps(end)
              ...
              for kk = 1 : size(steps,2)
                if dir_img(ii,jj) > (steps(kk)-11.25) && dir_img(ii,jj) < (steps(kk)+11.25)
                    dirQ(ii,jj) = steps(kk);
                end
              end
          end          
